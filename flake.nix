      ...
      mkSettings = { pkgs, desktopProfile ? { } }:
        let
          ...
          barEnd = if batteryOnBar
            then ''end = ["media", "recorder", "bar", "notifications", "battery", "session"]''
            else ''end = ["media", "recorder", "bar", "notifications", "session"]'';
        in
        pkgs.writeText "noctalia-config.toml" (builtins.replaceStrings
          [
            ...
            "end = [\"media\", \"recorder\", \"bar\", \"notifications\", \"session\"]"
          ]
          [
